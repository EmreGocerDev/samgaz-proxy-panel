...
function LoginButton() {
  ...
  return (
    ...
    >
      {pending ? ( <div className="w-5 h-5 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin"></div> ) : (
        <>
          ...
          <span className="relative flex justify-center items-center h-5 overflow-hidden">
            {buttonText.split("").map((letter, i) => (<span key={i} className="inline-block transition-transform duration-300 ease-in-out group-hover:-translate-y-full" style={{ transitionDelay: `${i * 30}ms` }}>{letter === " " ? "\u00A0" : letter}</span>))}
            <span className="absolute flex justify-center items-center">
              {buttonText.split("").map((letter, i) => (<span key={i} className="inline-block transition-transform duration-300 ease-in-out translate-y-full group-hover:translate-y-0" style={{ transitionDelay: `${i * 30}ms` }}>{letter === " " ? "\u00A0" : letter}</span>))}
            </span>
          </span>
        </>
      )}
